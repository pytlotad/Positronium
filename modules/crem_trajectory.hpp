...
// reaction model, the initial principal level, deterministic emission, the
// ground-state emission floor, spin quantization and the Bohr photon energy
// -- moved to configuration_panel.hpp, which is now the one place a run is
// steered from.  This header and crem_collapse.hpp still read them by the
// same names, and still get them from the include below.
