...
int main(int argc,char** argv){
    ...
    const int dirs=argc>1?std::atoi(argv[1]):50;
    std::printf("# Accuracy::farFieldSampling.directionCount = %d\n",dirs);
    const auto go=[&](const State& start,double total,int steps,
        ...
        acc.farFieldSampling.directionCount=dirs;
    ...
    std::printf("%6s %7s %11s %7s %7s %13s %13s\n",
                ...
                "residual","matched?");
    for(double beta:{0.0,0.35,0.50,0.70}){
        const double g=1.0/std::sqrt(1.0-beta*beta);
        State moving=rest;
